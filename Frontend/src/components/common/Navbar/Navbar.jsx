import { useEffect, useState } from "react";
import { Dialog } from "@headlessui/react";
import { Bars3Icon, XMarkIcon } from "@heroicons/react/24/outline";
import { Link } from "react-router-dom";
import { checkUserToken, getlogginedStatus } from "../../Utility/authUtils";
import axios from "axios";

const patientnavigation = [
  { name: "Diagnose ", href: "/DiagnosTest" },
  { name: "1:1 AI Therapy", href: "chat" },
  { name: "Book Appointment", href: "FindDoctors" },
  { name: "About Us", href: "aboutus" },
];

const doctornavigation = [
  {
    name: "Check Appointment",
    href: "/appoinment",
  },
];
const adminnavigation = [
  {
    name: "User Manage",
    href: "/userManage",
  },
  {
    name: "Doctor Manage",
    href: "/doctorManage",
  },
];

const Navbar = () => {
  const logoutHandle = () => {
    localStorage.clear();
    window.location.href ="./login"
  };

  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  const [navigation,setNavigation] = useState([])
  const [userrole, setUsesRole] = useState();
  let status = getlogginedStatus();
  useEffect(() => {
    const token = checkUserToken();
    if (!token) {
      status = false;
      localStorage.clear();
    } else {
      axios
        .get("http://localhost:3000/user/profile", {
          headers: {
            "x-auth-token": localStorage.getItem("token"),
          },
        })
        .then((res) => {
          const role = res.data.role;
          setUsesRole(role);
          if(role === "Doctor"){
            setNavigation(doctornavigation)
          }
          else if(role === "Patient"){
            setNavigation(patientnavigation)
          }
          else if(role === "admin"){
            setNavigation(adminnavigation)
          }
          else
          {
            setNavigation([])
          }
        })
        .catch((e) => {
          toast.error("An error occurred while fetching profile data.");
        });
    }
  }, []);
  return (
    <header className="absolute inset-x-0 top-0 z-50">
      <nav
        className="flex items-center justify-between p-6 mx-10 lg:px-8"
        aria-label="Global"
      >
        <div className="flex lg:flex-1">
          <Link to="/" className="-m-1.5 p-1.5">
            <span className="sr-only">Your Company</span>
            <Link
              to="/"
              className="-mx-3 block rounded-lg px-3 py-2 text-base font-semibold leading-7 text-gray-900"
            >
              MindCare.
            </Link>
          </Link>
        </div>
        <div className="flex lg:hidden">
          <button
            type="button"
            className="-m-2.5 inline-flex items-center justify-center rounded-md p-2.5 text-gray-700"
            onClick={() => setMobileMenuOpen(true)}
          >
            <span className="sr-only">Open main menu</span>
            <Bars3Icon className="h-6 w-6" aria-hidden="true" />
          </button>
        </div>
        <div className="hidden lg:flex lg:gap-x-12">
          {status
            ? navigation.map((item) => (
                <a
                  key={item.name}
                  href={item.href}
                  className="text-sm font-semibold leading-6 text-gray-900"
                >
                  {item.name}
                </a>
              ))
            : null}
        </div>
        {!status ? (
          <div className="hidden lg:flex lg:flex-1 lg:justify-end">
            <Link
              to="/login"
              className="text-sm font-semibold leading-6 text-gray-900"
            >
              Log in <span aria-hidden="true">&rarr;</span>
            </Link>
          </div>
        ) : (
          <>
          <div className="hidden lg:flex lg:flex-1 lg:justify-end">
            <Link
              to="/login"
              className="text-sm font-semibold leading-6 text-gray-900"
            onClick={()=>logoutHandle()}
            >
              log out <span aria-hidden="true">&rarr;</span>
            </Link>
          </div>
          <Link to="/ProfilePage">
            <div className="p-2 m-2 hover:bg-gray-300 rounded-lg">
              <img src="images\1.jpg" className="w-8 rounded-full" />
            </div>
          </Link>
          </>
        )}
      </nav>
      <Dialog
        as="div"
        className="lg:hidden"
        open={mobileMenuOpen}
        onClose={setMobileMenuOpen}
      >
        <div className="fixed inset-0 z-50" />
        <Dialog.Panel className="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-white px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
          <div className="flex items-center justify-between">
            <a href="#" className="-m-1.5 p-1.5">
              <span className="sr-only">Your Company</span>
              <img className="h-8 w-auto" src="images/logo.png" alt="" />
            </a>
            <button
              type="button"
              className="-m-2.5 rounded-md p-2.5 text-gray-700"
              onClick={() => setMobileMenuOpen(false)}
            >
              <span className="sr-only">Close menu</span>
              <XMarkIcon className="h-6 w-6" aria-hidden="true" />
            </button>
          </div>
          <div className="mt-6 flow-root">
            <div className="-my-6 divide-y divide-gray-500/10">
              <div className="space-y-2 py-6">
                {status
                  ? navigation.map((item) => (
                      <a
                        key={item.name}
                        href={item.href}
                        className="-mx-3 block rounded-lg px-3 py-2 text-base font-semibold leading-7 text-gray-900 hover:bg-gray-50"
                      >
                        {item.name}
                      </a>
                    ))
                  : null}
              </div>
              {!status ? (
                <div className="py-6">
                  <Link
                    to="./login"
                    className="-mx-3 block rounded-lg px-3 py-2.5 text-base font-semibold leading-7 text-gray-900 hover:bg-gray-50"
                  >
                    Log in
                  </Link>
                </div>
              ) : (
                <div className="py-6">
                  <Link
                    className="-mx-3 block rounded-lg px-3 py-2.5 text-base font-semibold leading-7 text-gray-900 hover:bg-gray-50"
                    onClick={() => logoutHandle()}
                  >
                    Log out
                  </Link>
                </div>
              )}
              {/*  */}
            </div>
          </div>
        </Dialog.Panel>
      </Dialog>
    </header>
  );
};

export default Navbar;
