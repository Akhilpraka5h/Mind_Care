...
const DoctorsPage = () => {
  ...
  const [doctorStatusDetails,setDoctorStatusDetails] = useState([])
  ...
  useEffect(() => {
    ...
    const getDoctorStatus = async () => {
      const response = await axios.get("http://localhost:3000/patient/reqstatus", {
        headers: {
          "x-auth-token": localStorage.getItem("token"),
        },
      });
      setDoctorStatusDetails(response.data || []); // Set default value to empty array if undefined
    };
  ...
  return (
    <div className="Doctors_main_container ">
      ...
      <div className="Doctors_body_section">
        {console.log(allDoctors,doctorsIds)}
        {allDoctors.map((doctor, index) => (
          <DoctorsCard
            ...
            appr={doctorStatusDetails.find(data=>data.drId === doctor._id)}
          />
        ))}
      </div>
      <Footer />
